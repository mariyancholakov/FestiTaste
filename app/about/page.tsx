  ...
  return (
    <div className="pb-24">
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="rounded-[36px] bg-gradient-to-br from-cream via-white to-accent/20 px-6 py-14 sm:px-10">
          <SectionTitle
            eyebrow="About"
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8">
        <article className="rounded-[32px] border border-dark-green/10 bg-white p-8 shadow-[0_18px_50px_rgba(27,67,50,0.06)]">
          <p className="text-sm font-semibold uppercase tracking-[0.35em] text-primary">
            Нашата история
          </p>
          <h2 className="mt-4 font-display text-3xl font-bold text-dark-green">
            От идея за вкусни уикенди до curated платформа за събития
          </h2>
          <p className="mt-5 text-base leading-8 text-dark-green/70">
            ...
            подбора, организацията и вдъхновяващото представяне в основна мисия
            на бранда.
          </p>
          <p className="mt-4 text-base leading-8 text-dark-green/70">
            Днес комбинираме фестивали, специални оферти и локални истории в
            платформа, която помага на хората да избират по-лесно и да
            пътуват по-вкусно.
          </p>
        </article>

        <aside className="rounded-[32px] bg-dark-green p-8 text-cream shadow-[0_18px_50px_rgba(27,67,50,0.18)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-cream/70">София, България</p>
            </div>
          </div>
          <div className="mt-8 space-y-4 text-sm leading-7 text-cream/82">
            {contactProfile.map((item) => (
              <p key={item}>{item}</p>
            ))}
          </div>
          <div className="mt-8 flex items-center gap-3 rounded-[24px] bg-white/10 p-4">
            <PhoneCall size={18} />
            <p className="text-sm">
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
          {stats.map((stat) => (
            <article
              key={stat.label}
              className="rounded-[28px] border border-primary/10 bg-gradient-to-br from-white to-cream p-6 text-center shadow-[0_18px_50px_rgba(27,67,50,0.05)]"
            >
              <p className="font-display text-5xl font-bold text-primary">
                {stat.value}
              </p>
              <p className="mt-3 text-sm uppercase tracking-[0.3em] text-dark-green/55">
                {stat.label}
              </p>
            </article>
          ))}
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {team.map((member) => (
            ...
            >
              ...
              <p className="mt-4 text-sm leading-7 text-dark-green/70">
                {member.bio}
              </p>
            </article>
          ))}
